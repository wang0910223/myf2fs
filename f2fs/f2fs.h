...
#define F2FS_MOUNT_SWAP_FS_OPS		0x10000000
...
static inline bool __f2fs_is_dot_dotdot(const u8 *name, size_t len)
{
	if (len == 1 && name[0] == '.')
		return true;
...
#define is_dot_dotdot(name, len) __f2fs_is_dot_dotdot((const u8 *)(name), len)
...
static inline bool f2fs_hw_is_readonly(struct f2fs_sb_info *sbi)
{
	...
	if (!f2fs_is_multi_device(sbi)) {
		/* --- CXL DAX NATIVE MOD: 記憶體沒有唯讀屬性，直接回傳 false --- */
	...
	for (i = 0; i < sbi->s_ndevs; i++) {
		/* --- CXL DAX NATIVE MOD: 跳過 Device 0 (CXL) 的硬體唯讀檢查 --- */
