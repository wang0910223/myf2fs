 ...
 * 1) /sys/fs/myf2fs/features
 *   : shows runtime features supported by in-kernel f2fs along with Kconfig.
 *     - ref. F2FS_FEATURE_RO_ATTR()
 *
 * 2) /sys/fs/myf2fs/$s_id/features <deprecated>
 *   : shows on-disk features enabled by mkfs.f2fs, used for old kernels. This
 *     won't add new feature anymore, and thus, users should check entries in 3)
 *     instead of this 2).
 *
 * 3) /sys/fs/myf2fs/$s_id/feature_list
...
int __init f2fs_init_sysfs(void)
{
	...
	kobject_set_name(&f2fs_kset.kobj, "myf2fs");
	...
	f2fs_proc_root = proc_mkdir("fs/myf2fs", NULL);
	if (!f2fs_proc_root) {
		ret = -ENOMEM;
		goto put_kobject;
	}
...
void f2fs_exit_sysfs(void)
{
	kobject_put(&f2fs_feat);
	kset_unregister(&f2fs_kset);
	remove_proc_entry("fs/myf2fs", NULL);
	f2fs_proc_root = NULL;
}
