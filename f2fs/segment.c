				...
				struct block_device *bdev)
{
    ...
    /* --- CXL DAX NATIVE MOD: 攔截並跳過 Device 0 (CXL) 的 Flush --- */
    if (sbi->is_cxl_dax && bdev == NULL) {
        /* * 1. CXL 是記憶體直連，不具備傳統硬碟的 Write Cache 概念。
         * 2. 我們在 data.c 已經使用了 memcpy_flushcache，資料已經確保持久化。
         * 3. 這裡直接回傳 0 (成功)，避免對 NULL 指標執行 blkdev_issue_flush。
         */
        return 0;
    }
    /* ----------------------------------------------------------- */
    ...
    /* * 注意：trace_f2fs_issue_flush 內部可能會讀取 bdev 名稱，
     * 如果 bdev 為 NULL 可能也會報錯，保險起見我們也把 trace 包起來。
     */
    if (bdev) {
				...
				struct discard_cmd *dc, int *issued)
{
    ...
    if (sbi->is_cxl_dax && bdev == NULL) {
        ...
        trace_f2fs_issue_discard(bdev, dc->di.start, dc->di.len);  // debug info.
