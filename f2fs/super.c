	...
	while ((p = strsep(&options, ",")) != NULL) {
        ...
        case Opt_cxl_phys:
            ...
            if (!name)
                return -ENOMEM;
            sbi->is_cxl_dax = true;
            sbi->cxl_phys_addr = simple_strtoull(name, NULL, 16); // 16進位解析
            kfree(name);
            break;
...
static void f2fs_put_super(struct super_block *sb)
{
	...
	/* --- CXL DAX NATIVE MOD: 在釋放 sbi 之前，解除記憶體映射 --- */
	...
	/* --- CXL NATIVE MOD: 卸載時同樣不可釋放 CXL 的 raw_super --- */
    if (!sbi->is_cxl_dax)
        kfree(sbi->raw_super);
    /* ----------------------------------------------------------- */
	// kfree(sbi->raw_super);
...
static int f2fs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	/* --- CXL DAX NATIVE MOD: 避免在 CXL 模式下讀取 s_bdev --- */
	if (sbi->is_cxl_dax || !sb->s_bdev)
...
static void default_options(struct f2fs_sb_info *sbi, bool remount)
{
	...
	if (!remount) {
		...
		/* --- CXL DAX NATIVE MOD: 繞過硬體探測，避免 NULL Pointer Crash --- */
        if (sbi->is_cxl_dax) {
            // CXL 記憶體直接支援 Discard 邏輯（雖然不寫入硬體，但讓 FS 邏輯跑通）
            set_opt(sbi, DISCARD);
        } else {
	...
	/* --- CXL DAX NATIVE MOD: 只有非 CXL 模式才設定 SB_INLINECRYPT --- */
    if (!sbi->is_cxl_dax)
...
int f2fs_commit_super(struct f2fs_sb_info *sbi, bool recover)
{
	...
	/* --- CXL DAX NATIVE MOD: 繞過 Buffer Head，直接寫入 CXL 記憶體 --- */
    if (sbi->is_cxl_dax) {
        // 計算兩份 Superblock 在 CXL 上的絕對記憶體位址
        void *cxl_sb0 = (char *)sbi->cxl_base_addr + F2FS_SUPER_OFFSET;
        void *cxl_sb1 = (char *)sbi->cxl_base_addr + F2FS_SUPER_OFFSET + F2FS_BLKSIZE;
        
        void *backup_sb  = sbi->valid_super_block ? cxl_sb0 : cxl_sb1;
        void *current_sb = sbi->valid_super_block ? cxl_sb1 : cxl_sb0;

        // 1. 先寫入 Backup Superblock
        memcpy_flushcache(backup_sb, F2FS_RAW_SUPER(sbi), sizeof(struct f2fs_super_block));

        // 2. 如果是 recovery 模式，寫完 Backup 就結束
        if (recover)
            return 0;

        // 3. 寫入 Current Superblock
        memcpy_flushcache(current_sb, F2FS_RAW_SUPER(sbi), sizeof(struct f2fs_super_block));
        
        return 0; // 成功回傳，不進入下方的 sb_bread
    }
    /* ------------------------------------------------------------- */
...
static int f2fs_scan_devices(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < max_devices; i++) {
        ...
        if (sbi->is_cxl_dax && i == 0) {
            FDEV(i).bdev_handle = NULL;
            FDEV(i).bdev = NULL; // CXL 沒有 block_device 結構
            ...
            sbi->s_ndevs = i + 1;
            f2fs_info(sbi, "Mount Device [%2d]: %20s (Native CXL Memory)", i, FDEV(i).path);
            continue; // 直接跳過底下的 bdev 操作
        }
        /* ----------------------------------------------- */
...
try_onemore:
    ...
    if (sb->s_bdev == NULL) {
        ...
        // 【極度重要】手動設定 Block Size，避免底下呼叫 sb_set_blocksize 導致 Kernel Crash
        sb->s_blocksize = F2FS_BLKSIZE;
        sb->s_blocksize_bits = F2FS_BLKSIZE_BITS;

        // TODO: 在這裡 (或是 mount 入口處) 必須透過 devm_memremap 取得實體記憶體映射
        // 假設你已經取得了記憶體指標並存入 sbi->cxl_base_addr
        // sbi->cxl_base_addr = devm_memremap(...);

		// 1. 從 mount options (傳入的 data 字串) 提前解析實體位址與大小
        if (data) {
            char *p;
            // 解析實體位址 (16進位)
        ...
        // 2. 防呆檢查
        if (!sbi->cxl_phys_addr || !sbi->cxl_size) {
            f2fs_err(sbi, "CXL mode requires cxl_phys and cxl_size mount options!");
            err = -EINVAL;
            goto free_sbi;
        }

        // 3. 正式將 CXL 實體位址映射到 Kernel 虛擬位址 
        // 使用 MEMREMAP_WB (Write-Back) 模式，這與 CXL 的 Cache-coherent 特性完美契合
	...
	/* --- NEW: 2. 只有非 CXL 模式才呼叫原生的 sb_set_blocksize --- */
    if (!sbi->is_cxl_dax) {
	...
	/* Deleted by CXL DAX implementation */
	// /* set a block size */
	// if (unlikely(!sb_set_blocksize(sb, F2FS_BLKSIZE))) {
	// 	f2fs_err(sbi, "unable to set blocksize");
	// 	goto free_sbi;
...
free_options:
	...
	kvfree(options);
free_sb_buf:
	/* --- CXL DAX NATIVE MOD: CXL 的 raw_super 是硬體映射的，絕對不能 kfree --- */
    if (!sbi->is_cxl_dax)
        kfree(raw_super);
    /* -------------------------------------------------------------------- */
free_sbi:
